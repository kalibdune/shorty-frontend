...
const UrlCard: React.FC<UrlCardProps> = ({ url, onDelete }: UrlCardProps) => {
	...
	const [editedExpiredAt, setEditedExpiredAt] = useState(
		url.expired_at ? url.expired_at : 'Безлимитно'
	)
	...
	const handleConfirmChanges = () => {
		...
		const isValidDate =
			editedExpiredAt === 'Безлимитно' || validateDate(editedExpiredAt)
		...
		if (url.url !== editedUrl || url.expired_at !== editedExpiredAt) {
			const payload: UrlUpdateRequest = {
				...
				expired_at:
					editedExpiredAt === 'Безлимитно'
						? null
						: getExpField(ExpOpts.Custom, editedExpiredAt),
			}
			console.log(payload)
			api
				.updateUrlById(url.id, payload)
				.then(() => {
					url.url = editedUrl
	...
	return (
		<>
			...
			{isEditing && (
				<div className='overlay' onClick={handleCancelChanges}></div>
			)}

			<div className={`card ${isEditing && 'above'}`} data-id={url.id}>
				<div
					className='card-header styled-link'
					onClick={() => {
						setIsOpenPopup(true)
						setPopupChildren(
							<QRPopup
								url={`${BASE_URL}/${url.hash}`}
								hashCode={url.hash}
							></QRPopup>
						)
					}}
							...
							<div className='card-body'>
								...
								<Button
									onClick={() => {
										setIsOpenPopup(true)
										setPopupChildren(
											<StatisticsPopup urlId={url.id}></StatisticsPopup>
										)
									}}
									className='card-button'
								>
