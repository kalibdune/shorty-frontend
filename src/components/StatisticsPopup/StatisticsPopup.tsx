...
const StatisticsPopup: React.FC<StatisticsChartProps> = ({ urlId }) => {
	...
	useEffect(() => {
		...
		apiRef.current
			...
			.catch((err) => {
				...
				setError(
					'Не удалось загрузить данные статистики. Пожалуйста, попробуйте позже.'
				)
			})
			.finally(() => {
				setIsLoading(false)
	...
	const chartData = useMemo(() => {
		if (
			!apiData ||
			!apiData.url_redirections ||
			apiData.url_redirections.length === 0
		) {
			return []
		}
		const now = new Date()
		...
		const result: HourlyData[] = Array.from(hourlyMap.entries()).map(
			([hour, count]) => ({
				hour,
				count,
			})
		...
		<div className='statistics-chart'>
				...
				{!isLoading && !error && chartData.length > 0 ? (
					...
					!error && (
						<div className='no-data'>
							Нет данных о переходах для отображения в выбранном диапазоне
						</div>
					)
				)}
			</div>
			...
			{apiData && apiData.count > 0 && (
				<div className='statistics-summary'>
					<p>
						Всего переходов: <strong>{apiData.count}</strong>
					</p>
				</div>
			)}
		</div>
